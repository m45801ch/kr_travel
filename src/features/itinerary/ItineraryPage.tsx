...
export function ItineraryPage() {
  ...
  const startEditDates = () => {
    if (!trip) return
    setStartDateDraft(trip.startDate); setEndDateDraft(trip.endDate); setEditingDates(true)
  }
  const saveDates = async () => {
    if (!trip) return
    const isoPattern = /^\d{4}-\d{2}-\d{2}$/
    const sanitize = (value: string, fallback: string) => (isoPattern.test(value) ? value : fallback)
    ...
    // 依新日期範圍重建天次:保留既有天次的標題與說明,超出範圍的天次與活動一併清除
    const nextDays = makeDays(next)
    for (const day of days) {
      await repository.deleteActivitiesByDay(day.id)
      await repository.saveDay({ ...day, tripId: next.id })
    }
    await Promise.all(nextDays.map((day) => repository.saveDay(day)))
      ...
      <div className="trip-heading">
        ...
        {editingDates
          ? <div className="inline-edit date-inline-edit">
              <input type="date" className="inline-edit-input" value={startDateDraft} min={trip.startDate} max={trip.endDate || undefined} onChange={(event) => { if (/^\d{4}-\d{2}-\d{2}$/.test(event.target.value)) setStartDateDraft(event.target.value) }} aria-label="開始日期" />
              <span className="date-separator">—</span>
              <input type="date" className="inline-edit-input" value={endDateDraft} min={startDateDraft} onChange={(event) => { if (/^\d{4}-\d{2}-\d{2}$/.test(event.target.value)) setEndDateDraft(event.target.value) }} aria-label="結束日期" />
              <button type="button" className="inline-edit-ok" onClick={() => void saveDates()}>完成</button>
              <button type="button" className="inline-edit-cancel" onClick={() => setEditingDates(false)}>取消</button>
            </div>
