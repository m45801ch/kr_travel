...
const defaultSettings: Settings = { id: 'global', themeColor: '#ef8490', themeId: 'classic', fontScale: 1, darkMode: false, effects: false, defaultCurrency: 'TWD', mapProvider: 'google' }

export function SettingsPage() {
  const [settings, setSettings] = useState<Settings>(defaultSettings)
  useEffect(() => { void db.settings.get('global').then((saved) => { if (saved) setSettings({ ...defaultSettings, ...saved }) }) }, [])
