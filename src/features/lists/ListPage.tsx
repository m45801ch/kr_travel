...
=======
    return <section className="list-page"><header className={`page-header themed-header ${type === 'shopping' ? 'themed-header-shopping' : 'themed-header-prep'}`}><ThemeHeaderArt kind={type === 'shopping' ? 'shopping' : 'prep'} /><div>
<p className="eyebrow">{type === 'shopping' ? 'SHOPPING LIST' : 'BEFORE TRIP'}</p><h1>{title}</h1><p>{subtitle}</p></div></header><ProgressSummary total={items.length} completed={completed} /><div className="list-filter"><div className="list-filter-tabs">{categories.map((item) => <button className={filter === item ? 'is-active' : ''} type="button" key={item} onClick={() => setFilter(item)}>{item}</button>)}</div><button className="list-filter-add" type="button" onClick={() => { setEditingItem(undefined); setShowForm(true) }}><Plus size={16} />新增</button></div><div className="list-items">{visible.length ? visible.map((item) => <ListItemCard key={item.id} item={item} onToggle={async (id) => { const target = items.find((item) => item.id === id); if (target) { await listRepository.save({ ...target, completed: !target.completed }); await reload() } }} onEdit={(id) => { const target = items.find((item) => item.id === id); if (target) { setEditingItem(target); setShowForm(true) } }} onDelete={async (id) => { const target = items.find((it) => it.id === id); if (target) for (const pid of getAllPhotoIds(target)) await deletePhoto(pid); await listRepository.delete(id); await reload() }} />) : <div className="empty-activities">還沒有項目，按下新增開始整理吧。</div>}</div>{showForm && <ListItemForm type={type} tripId={trip.id} members={members} initial={editingItem} onSave={async (item) => { await listRepository.save(item); setShowForm(false); setEditingItem(undefined); await reload() }} onDelete={async (id) => { const target = items.find((it) => it.id === id) ?? editingItem; if (target) for (const pid of getAllPhotoIds(target)) await deletePhoto(pid); await listRepository.delete(id); setShowForm(false); setEditingItem(undefined); await reload() }} onCancel={() => { setShowForm(false); setEditingItem(undefined) }} />}</section>
>>>>>>> 40e87a4 (feat: refresh themed header illustrations)
}
